...
const filtered = computed(() => {
  return transactions.value.filter(t => {
    ...
    if (filterCat.value !== 'all' && t.category_id !== filterCat.value) return false;
    if (filterWallet.value !== 'all' && t.wallet_id !== filterWallet.value) return false;
  ...
  <div class="flex flex-col gap-5">
    ...
    <div class="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
      ...
      <div class="md:hidden divide-y divide-border">
        <div v-if="filtered.length === 0" class="text-center py-16 text-muted-foreground text-sm">Tidak ada transaksi</div>
        <div v-for="tx in filtered" :key="tx.id" class="flex items-center gap-3 px-4 py-3.5">
